...
export default function MemberPanel({
  ...
  if (!sessionMemberId || !members.some(m => m.id === sessionMemberId && m.password === m.password)) {
    // Check if previously logged in but mismatch, let's reset it safely
    const isMemberValid = members.some(m => m.id === sessionMemberId);
    const activeId = isMemberValid ? sessionMemberId : (members[0]?.id || "");
    ...
    return (
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden max-w-md mx-auto my-4 font-sans" id="member-login-card">
        <div className="p-6 bg-slate-50/50 border-b border-secondary/10 text-center">
        ...
        <form onSubmit={handleLoginSubmit} className="p-6 space-y-4 text-right">
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-500 mb-1">نام کاربری خود را انتخاب کنید:</label>
            <select
              value={activeId}
              onChange={(e) => {
          ...
          <button
            type="button"
            onClick={() => {
              // Bypass custom credentials block dynamically if they forgot
              const testId = sessionMemberId || members[0]?.id;
              const target = members.find(m => m.id === testId);
              if (target) {
                setSessionMemberId(target.id);
                setTypedPassword(target.password);
  ...
  let simulatedScoreCalculations = calculatePaymentScore(simulatedDay, totalAmountToCalculateScore, settings.lotteryDayOfMonth);
  ...
  // Received Loans (تفکیک شده) - Loans received by this member
  // Since we don't have lotteries in props, we can extract wins where hasWon is true
  // Let's filter payments or winnings: we can display how many times hasWon is true and winMonth is specified!
  // To show received amounts:
  // - If member hasWon is true: they received a Main Loan in they winMonth which equals settings.monthlyAmount * members.length.
  //   Let's render a beautiful card for this received amount!
  // Same for any other custom emergency loans if registered.
  const mainLoanReceivedAmount = activeMember.hasWon ? (members.length * settings.monthlyAmount) : 0;
            ...
            {isPaidThisMonth && currentMonthPayment ? (
              ...
              <div className="p-6 bg-teal-50/20 rounded-xl border border-teal-150 text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    پرداختی شما به ارزش مجموع <b>{formatCurrency(settings.monthlyAmount + (settings.savingsAmount || 500000))}</b> در تاریخ {currentMonthPayment.paymentDateShamsi} (روز {toPersianDigits(currentMonthPayment.paymentDayShamsi)}ام ماه) در دفاتر صندوق به ثبت رسیده است.
                  </p>
                </div>
              ...
              <div className="space-y-4" id="member-unpaid-workspace">
                <div className="p-4 bg-rose-50/40 rounded-lg border border-rose-100 flex items-start gap-3">
                  ...
                  <div>
                    <h5 className="text-xs font-bold text-rose-950">تعهد واریز این ماه باز است!</h5>
                    <p className="text-[10px] text-slate-500 mt-1 leading-relaxed">
                      حق قسط ماهیانه این دوره {formatCurrency(settings.monthlyAmount)} به همراه اندوخته پس‌انداز {formatCurrency(settings.savingsAmount || 500000)} است.
                    </p>
                  </div>
                </div>
                ...
                >
                  ...
                  <span>ثبت و واریز قسط ماهیانه {formatCurrency(settings.monthlyAmount + (settings.savingsAmount || 500000))}</span>
                </button>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-4">
            <div className="flex items-center gap-3 pb-3 border-b border-slate-200">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-mono mt-0.5">{toPersianDigits(activeMember.phone)}</p>
              </div>
            </div>
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-[9px] text-slate-500 mt-0.5 block">مبلغ قابل واگذاری: {formatCurrency(members.length * settings.monthlyAmount)}</span>
                  </div>
                  <div>
                    {activeMember.hasWon ? (
